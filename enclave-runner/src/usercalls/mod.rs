...
use async_queue::{self, Fifo, QueueEvent};
use failure;
use fnv::FnvHashMap;
            ...
            Ok(ret) => {
                match handle_data {
                    ...
                    UsercallHandleData::Async(usercall, return_queue_descriptor, sync_usercall_tx) => {
                        ...
                        let return_queue_tx = async_queue::AsyncSender::new(return_queue_descriptor, synchronizer);
                        let ret = Return {
                            id: usercall.id,
                            value: ret,
                        };
...
impl<'tcs> IOHandlerInput<'tcs> {
    ...
    async fn wait(&mut self, event_mask: u64, timeout: u64) -> IoResult<u64> {
        ...
        if ret.is_none() {
            loop {
                let ev = if wait {
                    Ok(tcs.event_queue.next().await.unwrap())
                } else {
                    match tcs.event_queue.try_next() {
                        Ok(Some(ev)) => Ok(ev),
                        Err(_) => break,
                        Ok(None) => Err(()),
                    }
...
impl async_queue::AsyncSynchronizer for QueueSynchronizer {
    fn wait(&self, event: QueueEvent) -> Pin<Box<dyn Future<Output = StdResult<(), async_queue::SynchronizationError>> + '_>> {
        ...
        async move {
            ...
            match subscription.recv().await {
                ...
                Err(async_pubsub::RecvError::Closed) => Err(async_queue::SynchronizationError::ChannelClosed),
            }
        }.boxed_local()
    }
